...
const groupMapping = {
  1: '120363417941205146@g.us', // Módulo 6 o erro de conexão pode ser por falta dos grupos no novo whatsapp
...
// Conjunto de clientes WebSocket
...
async function getConfig(setting) {
  ...
  try {
    const result = await pool.query('SELECT setting, value FROM chatbot_config WHERE setting IN ($1, $2, $3)', [
...
function initializeChatbot() {
  ...
  client.on('message', async msg => {
    ...
    if (msg.body.toLowerCase() === 'ativar') {
    ...
    if (menuEnabled !== '1' && ['1', '2', '3', '4'].includes(msg.body.trim())) {
    ...
    switch (userState.state) {
      case 'IDLE':
        if (msg.body.match(/(oi|Olá|olá|ola|Ola)/i)) {
          await showMainMenu(msg, name);
          return;
        }
      ...
      case 'AWAITING_MODULO_ID':
        const moduloId = parseInt(msg.body.trim());
        if (isNaN(moduloId)) {
      ...
      case 'AWAITING_AVALIACAO_ID':
        const avaliacaoId = parseInt(msg.body.trim());
      ...
      case 'AWAITING_PROFESSOR_ID':
        const professorId = parseInt(msg.body.trim());
...
async function sendNotifications() {
  ...
  try {
    ...
    console.log(`Processadas ${notifications.length} notificações para até ${Object.keys(groupMapping).length + 1} grupos.`);
  } catch (error) {
...
function startNotificationRoutine() {
  setInterval(sendNotifications, 60000); // 1 minuto (ajustar para 1h30min depois)
  setInterval(updateCache, 3600000); // Atualiza cache a cada hora
  setInterval(updateStatus, 60000); // Persiste status a cada minuto
  sendNotifications();
}
...
async function sendBroadcastMessage(message, groupIds) {
  ...
  // Opcional: Verificar auto_replies_enabled para broadcasts
  const autoRepliesEnabled = await getConfig('auto_replies_enabled');
  if (autoRepliesEnabled !== '1') {
  ...
  const validGroupIds = groupIds.filter(id => groupChatIds[id]);
