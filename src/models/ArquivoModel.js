...
//       console.log('Avaliações arquivadas encontradas:', avaliacoes);
//       return avaliacoes;
//     } catch (error) {
//       console.error('Erro ao buscar avaliações arquivadas:', error.message, error.stack);
//       throw new Error(`Erro na consulta ao banco de dados: ${error.message}`);
...
//     try {
//       const result = await pool.query(
//         `UPDATE avaliacao SET delete_logico = FALSE, updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING id`,
//         [id]
//       );
//       if (result.rowCount === 0) {
//         throw new Error('Avaliação não encontrada ou já restaurada.');
//       }
//       console.log(`Avaliação ID ${id} restaurada.`);
//       return result.rows[0];
//     } catch (error) {
//       console.error('Erro ao restaurar avaliação:', error.message);
//       throw error;
//     }
//   }
...
//       const result = await pool.query('DELETE FROM avaliacao WHERE id = $1 RETURNING id', [id]);
//       if (result.rowCount === 0) {
...
//     } catch (error) {
//       await pool.query('ROLLBACK');
//       console.error('Erro ao excluir avaliação permanentemente:', error.message);
//       throw error;
//     }
//   }
// }
